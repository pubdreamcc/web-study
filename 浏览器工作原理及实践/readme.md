...
* https：数据传输更加安全
> http 是明文传输
> 在 http 和 tcp 之间 加上一个 安全层
> 对称加密
> 浏览器 和 服务器 使用相同的 密钥
> 缺点：传输过程中的随机数 & 加密套件是 明文，可以被获取
> 非对称加密
> 浏览器使用公钥加密，服务器使用 私钥解密，服务器私钥加密返回数据，浏览器公钥解密数据
> 缺点：效率低，服务端返回的数据 不安全，浏览器发送的公钥可以被获取，导致服务端返回数据被篡改

> 对称加密 & 非对称加密 搭配使用
> 传输过程中的数据使用对称加密，对称加密的密钥使用非对称加密来传输
